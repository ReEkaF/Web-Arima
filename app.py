import matplotlib
matplotlib.use('Agg')  # Ensure Matplotlib does not use a GUI backend like Tkinter
...
def create_acf_plot(residuals):
    ...
    acf_filename = f'acf.jpg'  # Nama file unik dengan UUID
...
def create_pacf_plot(residuals):
    ...
    pacf_filename = f'pacf.jpg'  # Nama file unik dengan UUID
...
def admin_arima_manual_parameter():
    ...
    if request.method == 'POST':
        try:
            ...
            img_filename = create_forecast_plot(data, forecast_df)      

            # Plot ACF dan PACF dari residuals
            acf_filename = create_acf_plot(model_fit.resid)
            pacf_filename = create_pacf_plot(model_fit.resid)
            return render_template('admin/hasil_manual.html', forecast=forecast, plot_url=img_filename, acf_url=acf_filename, pacf_url=pacf_filename, mape=mape, mae=mae, mse=mse, rmse=rmse)
